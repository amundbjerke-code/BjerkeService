...
export async function POST(request: Request, context: { params: Promise<{ avvikId: string }> }) {
  ...
  if (!parsedParams.success) {
    return NextResponse.json({ error: "Ugyldig avvik-id" }, { status: 400 });
  }

  const avvik = await db.avvik.findUnique({
    where: { id: parsedParams.data.avvikId },
    select: { id: true }
  });
  if (!avvik) {
  ...
  const uploadDir = path.join(process.cwd(), "public", "uploads", "avvik-attachments");
  ...
  for (const file of files) {
    ...
    const filUrl = `/uploads/avvik-attachments/${filename}`;
    const created = await db.avvikAttachment.create({
      data: {
        avvikId: avvik.id,
        filUrl,
  ...
  await logAudit({
    ...
    action: "AVVIK_ATTACHMENT_UPLOADED",
    entityType: "AVVIK",
    entityId: avvik.id,
    ipAddress: getRequestIp(request),
    metadata: { avvikId: avvik.id, attachmentCount: createdAttachments.length }
  });
